...
for (int i = 0; i < 8; i++) {
    ...
    // Открытие файла для записи
           int file1 = open("output1.txt", O_WRONLY|O_CREAT);
           if (file1 < 0) {
           	cout << "Ne otkryls"<<endl;
               // Обработка ошибки открытия файла
               return 1;
           }
           cout << "otrkytie +"<<endl;
           cout <<"Outbuf= "<<&output_buffer<<endl;
           // Запись буфера в файл
           ssize_t bytes_written = write(file1, output_buffer, j);
           cout << "++a"<<endl;
           if (bytes_written != j) {
